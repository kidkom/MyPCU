...
Public Class frmAdjNation
    Private Sub frmAdjNation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub ShowData()
        ...
        ElseIf chk0.Checked = True Then
            tmpSQL = " WHERE IFNULL(NATION_HOS,'') = '' "
        ElseIf chk1.Checked = True Then
            tmpSQL = " WHERE IFNULL(NATION_HOS,'') <> '' "
        End If
        ds = clsdataBus.Lc_Business.SELECT_TABLE(" a.NATION,b.NATION_DESC AS NATION_DESC,IFNULL(a.NATION_HOS,'') AS NATION_HOS,c.NATION_DESC AS NATION_DESC_HOS ", " m_person a JOIN l_nation b ON(a.NATION = b.NATION_CODE) LEFT JOIN l_mypcu_nation c ON(a.NATION_HOS = c.NATION_CODE) ", " " & tmpSQL & " GROUP BY a.NATION  ")
        If ds.Tables(0).Rows.Count > 0 Then
            Betterdisplay(ds)
    ...
    Private Sub Betterdisplay(ByVal ds As DataSet)
        ...
        Try
            ...
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                BetterListView1.Items.Add(dr("NATION").ToString).AlignHorizontal = TextAlignmentHorizontal.Center
                BetterListView1.Items(i).SubItems.Add(dr("NATION_DESC").ToString).AlignHorizontal = TextAlignmentHorizontal.Left
                BetterListView1.Items(i).SubItems.Add(dr("NATION_HOS").ToString).AlignHorizontal = TextAlignmentHorizontal.Center
                BetterListView1.Items(i).SubItems.Add(dr("NATION_DESC_HOS").ToString).AlignHorizontal = TextAlignmentHorizontal.Left
    ...
    Private Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        If lblNation.Text = "" Then
            XtraMessageBox.Show("กรุณากำหนดรหัสสัญชาติก่อน!!!", vProgram, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txtNationOld.Select()
            Exit Sub
        End If

        clsbusent.Lc_BusinessEntity.Updatem_table(" m_person ", "NATION_HOS = '" & lblNation.Text & "'", " NATION = '" & lblNationOld.Text & "'")
