...
Public Class frmAdjEducation
    Private Sub frmAdjEducation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ShowData()
    End Sub
    Private Sub CboEducationAction()
        Dim ds As DataSet
        ds = clsdataBus.Lc_Business.SELECT_TABLE("EDUCATION_DESC,EDUCATION_CODE", " l_mypcu_education ", " WHERE STATUS_AF = '1' ")
    ...
    Private Sub ShowData()
        ...
        ElseIf chk0.Checked = True Then
            tmpSQL = " WHERE IFNULL(EDUCATION_HOS,'') = '' "
        ElseIf chk1.Checked = True Then
            tmpSQL = " WHERE IFNULL(EDUCATION_HOS,'') <> '' "
        End If
        ds = clsdataBus.Lc_Business.SELECT_TABLE(" a.EDUCATION,b.EDUCATION_DESC AS EDUCATION_DESC,IFNULL(a.EDUCATION_HOS,'') AS EDUCATION_HOS,c.EDUCATION_DESC AS EDUCATION_DESC_HOS ", " m_person a JOIN l_education b ON(a.EDUCATION = b.EDUCATION_CODE) LEFT JOIN l_mypcu_education c ON(a.EDUCATION_HOS = c.EDUCATION_CODE) ", " " & tmpSQL & " GROUP BY a.EDUCATION  ")
        If ds.Tables(0).Rows.Count > 0 Then
            Betterdisplay(ds)
    ...
    Private Sub Betterdisplay(ByVal ds As DataSet)
        ...
        Try
            ...
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                BetterListView1.Items.Add(dr("EDUCATION").ToString).AlignHorizontal = TextAlignmentHorizontal.Center
                BetterListView1.Items(i).SubItems.Add(dr("EDUCATION_DESC").ToString).AlignHorizontal = TextAlignmentHorizontal.Left
                BetterListView1.Items(i).SubItems.Add(dr("EDUCATION_HOS").ToString).AlignHorizontal = TextAlignmentHorizontal.Center
                BetterListView1.Items(i).SubItems.Add(dr("EDUCATION_DESC_HOS").ToString).AlignHorizontal = TextAlignmentHorizontal.Left
    ...
    Private Sub BetterListView1_SelectedItemsChanged(sender As Object, eventArgs As BetterListViewSelectedItemsChangedEventArgs) Handles BetterListView1.SelectedItemsChanged
        For i As Integer = 0 To BetterListView1.SelectedItems.Count - 1
            ...
            lblEducationOld.Text = lvi.SubItems.Item(0).Text
            txtEducationOld.Text = lvi.SubItems.Item(1).Text
        Next
    End Sub
    Private Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        If lblEducation.Text = "" Then
            XtraMessageBox.Show("กรุณากำหนดรหัสการศึกษาก่อน!!!", vProgram, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            txtEducationOld.Select()
            Exit Sub
        End If

        clsbusent.Lc_BusinessEntity.Updatem_table(" m_person ", "EDUCATION_HOS = '" & lblEducation.Text & "'", " EDUCATION = '" & lblEducationOld.Text & "'")
